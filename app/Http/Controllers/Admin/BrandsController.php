...
class BrandsController extends Controller
{
    ...
    public function update(BrandFormRequest $request, $id)
    {
        ...
        if ($response->successful())
            return redirect('/admin/brands')->with('message', 'Proveedor actualizado con éxito.');
    ...
    public function destroy($brandId, Request $request)
    {
        ...
        if ($response->successful())
            return response()->json(['success' => true, 'message' => 'Proveedor eliminado con exito.' . $adminName . ' Si'], 200);
    ...
    public function store(BrandFormRequest $request)
    {
        ...
        if ($response->successful())
            return redirect('/admin/brands')->with('message', 'Proveedor agregado con éxito.');
